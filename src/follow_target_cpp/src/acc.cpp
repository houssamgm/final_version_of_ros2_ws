...
private:
    ...
    double leader_v_ = 0.0, leader_w_ = 0.0; // Store leader commands
    ...
    void control_loop()
    {
        ...
        // --- Start the Reach Timer on first successful TF lookup ---
        if (!first_tf_received_)
        {
            start_time_ = this->get_clock()->now();
            first_tf_received_ = true;
            RCLCPP_INFO(this->get_logger(), "TF Established. Starting Time-to-Reach stopwatch.");
        }
        ...
        // Get absolute coordinates relative to the 'odom' frame for trajectories
        try {
        ...
        if (!reached_setpoint_ && std::abs(gap_error) <= stop_threshold_)
        {
            ...
            // Calculate final time elapsed since first TF
            time_to_reach_ = (this->get_clock()->now() - start_time_).seconds();
